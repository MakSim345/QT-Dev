...
void MainClass::CalculateO2Volume()
{
  ...
  int nOxigenVolume = 0;
  /*
  nTmpVal = ui.IDC_O2Percents, NULL, 1);
  nOxigenVolume = ((float)nTmpVal/100) * (ui.IDC_TotalFlowResult, NULL, 1));
  //t.TRACEY ("nOxigenVolume: %ld. \n", nOxigenVolume);
  str.Format("%d", nOxigenVolume);
  ui.IDC_O2DAC, str);	
  */
}
...
void MainClass::OnConnect() 
{
	...
	if (0 == m_Connected)
	{
	 ...
	 if( PortObject->OpenPort(g_sComPort, 2048, INode, g_nComPortSpeed, TRUE	) )
	  {
	   // _beginthread(thReadSerialPort, 0, this );
	   m_Connected = 1;
