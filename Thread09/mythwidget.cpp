...
MyThWidget::MyThWidget(QWidget *parent)
     : QWidget(parent)
{
...
void MyThWidget::StartMyThread()
{
    m_thread.setMValue( lcd->intValue() );
    m_thread.start();
}

int MyThWidget::fib(int f)
{
    if (f<2)
    {
        return 1;
