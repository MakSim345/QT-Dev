...
extern CTRACE t;
extern DOIrisSerialPort *PortObject;
...
try 
 {
  switch(nSpeed)
		{
     case 9600:
			// brBitRate = CommPort::br9600;
			CPort[PortNumber] = new CommPort(PortNumber, CommPort::br9600);
			break;
		 case 19200:
			//brBitRate = CommPort::br19200
			CPort[PortNumber] = new CommPort(PortNumber, CommPort::br19200);
			break;
		 case 38400:
      //brBitRate = CommPort::br38400
			CPort[PortNumber] = new CommPort(PortNumber, CommPort::br38400);
		 	break;
		 case 57600:
			//brBitRate = CommPort::br57600
			CPort[PortNumber] = new CommPort(PortNumber, CommPort::br57600);
			break;
		 case 115200:
      //brBitRate = CommPort::br115200
...
int InitSerialPorts(int PortNumber)
  {
   int i = 0;
   //comm tmp_port;   
   // char IniFileName[15];
...
int ReadRxData(int PortNumber, char * RecvData)
  ...
  char MData[200];
  int c, j;
	unsigned long inCharsReq=0;
  int NbrOfReadBytes=0;

  //t.TRACEY("Enter to ReadRxData(). PortNumber=%d\n", PortNumber);
 ...
 {
	NbrOfReadBytes = CPort[PortNumber]->ReadFromPort(RecvData, inCharsReq);
 }
...
// check for capture
  ...
  //t.TRACEY (" return NbrOfReadBytes=%d\n", NbrOfReadBytes);
...
int TxData(int PortNumber, char * Data,int NbrOfBytes)

  {
 	 ...
 	 PortObject->WritePort(Data, NbrOfBytes);
   return 1;
...
//printf ("sending message from port %d\n", PortNumber);
 ...
 if (TRUE == ActivePort[PortNumber])
 {
  ...
  //{ 		
		...
		Ret = CPort[PortNumber]->WriteToPort(Data, NbrOfBytes);
		// t.TRACEY("TxData - sent %d bytes\n", Ret);
		if (!Ret)
    {
      // return(i);
			return(Ret);
