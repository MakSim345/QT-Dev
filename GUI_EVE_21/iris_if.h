...
#define LINK_TEST_SERIALIZED_MSG_SIZE 8 // remove later.

#define GET_REQUEST_STUFFED_MSG_SIZE ((GET_REQUEST_SERIALIZED_MSG_SIZE + MESSAGE_CLASS_SERIALIZED_BYTE_COUNT) * 2)
#define GET_REQUEST_SERIALIZED_MSG_SIZE_IRIS_NODE (GET_REQUEST_SERIALIZED_MSG_SIZE + MESSAGE_CLASS_SERIALIZED_BYTE_COUNT)

#define LINK_TEST_STUFFED_MSG_SIZE ((LINK_TEST_SERIALIZED_MSG_SIZE  + MESSAGE_CLASS_SERIALIZED_BYTE_COUNT) * 2)
#define LINK_TEST_SERIALIZED_MSG_SIZE_IRIS_NODE (LINK_TEST_SERIALIZED_MSG_SIZE + MESSAGE_CLASS_SERIALIZED_BYTE_COUNT)
...
class IRIS_Node : public IRIS_NodeInterface
{
  ...
  public:
     ...
     //virtual unsigned char GetRoute ();
		 int UpdateDialogItems(int nMsgNum, void* Dialog);
		 // this function moved from private:
	...
	protected:
      ...
      //int SendMessageRequest (int msgID);
       ...
       int IrisNodeInit (); // safe variant, for testing - YS, 13-OCT-2006
		   ...
		   int ReceiveMessage (Message *msg); // added YS, 18-JAN-2007, instead of pure virtual function
